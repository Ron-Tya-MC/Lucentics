package io.github.rontyamc.lucentics.blocks.injector;
...
public class InjectorBehavior extends BlockEntityBehavior implements Clearable, OutputReceiver {
    ...
    private final static RandomSource RANDOM_SOURCE = RandomSource.create();
    ...
    private PendingOutputs pendingOutputs = PendingOutputs.empty();
    ...
    private boolean craft(ServerLevel level, InjectingRecipe recipe, InjectingRecipeInput input) {
    ...
    public ItemStack acceptItem(ItemStack stack, boolean simulate) {
        ...
        if (buffer.isEmpty()) {
            ItemStack extracted = leftover.split(Math.min(stack.getMaxStackSize(), maxStackSize));

            if (!simulate) buffer = extracted;
        } else if (ItemUtil.isSameItem(buffer, stack, false)) {
            int insertCount = Math.min(getRemainingSpaceBuffer(), stack.getCount());
            leftover = stack.copyWithCount(stack.getCount() - insertCount);

            if (!simulate && insertCount > 0) buffer.grow(insertCount);
        }
        return leftover;
    }
    ...
    private void spawnCraftingParticles(ServerLevel level, BlockPos pos) {
        double Sx = pos.getX() + Mth.lerp(RANDOM_SOURCE.nextDouble(), 0.1, 0.9);
        double Sy = pos.getY() + 0.9;
        double Sz = pos.getZ() + Mth.lerp(RANDOM_SOURCE.nextDouble(), 0.1, 0.9);
    ...
    public void write(CompoundTag nbt, HolderLookup.Provider registries, boolean clientPacket) {
        ...
        pendingOutputs.write(nbt, registries, clientPacket);
    }
    ...
    public void read(CompoundTag nbt, HolderLookup.Provider registries, boolean clientPacket) {
        ...
        pendingOutputs.read(nbt, registries, clientPacket);
    }
